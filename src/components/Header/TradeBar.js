import { Arrow, CloseIcon } from "../../assets/svgs";
import "./Header.css";

export const TradeBar = ({ tradePriceData, showBalance, setShowBalance }) => {
  return (
    <div
    ...
    >
      ...
      <div className="tradeBarItem">
        ...
        <div className="tradeBarBalance">
          {tradePriceData?.freeMargin.currency}{" "}
          {tradePriceData?.freeMargin.value}
        </div>
      </div>
      <div className="tradeBarItem">
        <div className="tradeBarBalanceTitle">Used Margin</div>
        <div className="tradeBarBalance">
          {tradePriceData?.usedMargin.currency}{" "}
          {tradePriceData?.usedMargin.value}
        </div>
      </div>
      <div className="tradeBarItem">
      ...
      <div className="tradeBarItem">
        ...
        <div
          ...
          style={{
            color: tradePriceData?.marginLevel.dir
              ? "rgba(103, 190, 122, 1)"
              : "rgba(232, 116, 80, 1)",
          }}
        >
          <div
            ...
            style={{
              borderRight: tradePriceData?.marginLevel.dir
                ? "1px solid rgba(103, 190, 122, 1)"
                : "1px solid rgba(232, 116, 80, 1)",
              borderBottom: tradePriceData?.marginLevel.dir
                ? "1px solid rgba(103, 190, 122, 1)"
                : "1px solid rgba(232, 116, 80, 1)",
            }}
          ></div>
          {tradePriceData?.marginLevel.value}{" "}
          {tradePriceData?.marginLevel.measure}
        </div>
      </div>
      <Arrow className="closeTradeBar" onClick={() => setShowBalance(false)} />
    </div>
  );
};
