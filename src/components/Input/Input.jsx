          ...
          {!props.btns ? (
            ...
            <div className='input-form-frame-outer'>
              {props.btns &&
                props.btns.map(item => {
                  return (
                    <div onClick={props.onClick} className='input-form-frame' key={item.id}>
                      {item.value}
                    </div>
                  )
                })}
          ...
          >
            ...
            <svg
              className={`${active ? 'rotate' : ''} ${'arrow'} ${
                props?.value?.length > 0 && !edit && props.editable ? 'arrow-none' : 'arrow-show'
              } `}
              width='20'
              height='21'
              viewBox='0 0 20 21'
      ...
      <div style={props.customStyles} className='input-group-item phone-numbers relative'>
        ...
        <div
          className={`${'form-control'} ${'select-control'} ${props.emptyFieldErr ? 'error-border' : ''} ${
            !edit && props.editable && props.value?.number?.length > 0 ? 'disabled-input' : ''
          }`}
        >
          <div
            onClick={() => {
