...
export const CardSlider = ({ accounts, cardImgs, handleDeposit, handleWithdraw, handleExchange, handleTransfer }) => {
  ...
  const mainAcc = useMemo(() => {
    const item = accountsData && accountsData?.find(item => item?.account_category === accountType)
    return {
    ...
    <div className='card-slider-wrapper'>
      <div className='card-slider-navigation'>
        {accountsData?.map((item, index) => {
          const activeIndex = accountsData.findIndex(acc => accountType === acc.account_category)
          ...
          return (
            ...
            >
              ...
              >
                <p className='font-16'>
                  {item?.account_category === 'system' ? 'main' : item?.account_category} {width > 767 ? 'account' : ''}
                </p>
              </div>
            </div>
          )
